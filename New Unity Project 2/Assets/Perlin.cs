...
public class Perlin
{
	...
	public Perlin(int seed)
	{
		...
		UnityEngine.Random.seed = seed;// rnd = new UnityEngine.Random();

		//Debug.Log("calling correct perlin");
		for (i = 0 ; i < B ; i++) {
			p[i] = i;

			g1[i] = (float)(UnityEngine.Random.Range(0, B + B) - B) / B;
			//g1[i] = (float)(rnd.Next(B + B) - B) / B;
			
			for (j = 0 ; j < 2 ; j++)
				g2[i,j] = (float)(UnityEngine.Random.Range(0, B + B) - B) / B;
				//g2[i,j] = (float)(rnd.Next(B + B) - B) / B;
			normalize2(ref g2[i, 0], ref g2[i, 1]);
			
			for (j = 0 ; j < 3 ; j++)
				g3[i,j] = (float)(UnityEngine.Random.Range(0, B + B) - B) / B;
				//g3[i,j] = (float)(rnd.Next(B + B) - B) / B;
		...
		while (--i != 0) {
			k = p[i];
			p[i] = p[j = UnityEngine.Random.Range(0, B)];
			//p[i] = p[j = rnd.Next(B)];
			p[j] = k;
		}
